...
class gclda_dataset:
	...
	def importPeakIndices(self):
		...
		for line in fid:
			...
			tmp_peak_vals.append(list(map(float,linedat[1:]))) # Append the ndims remaining vals to a N x Ndims array ^^^If using different data (non-integer valued) 'int' needs to be changed to 'float'
